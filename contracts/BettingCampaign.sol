    ...
    ICampaign
{
    ...
    string private _uri=  "https://ipfs.io/ipfs/QmcDRWwXCE1LjvdESNZsmc75syTJP2zA8WW9SHEaCwEmkc/";
    ...
    function createCampaign(
        ...
        campaign_info[_totalCampaigns] = CampaignDetails(
            raceNum,
            raceDate,
            Stages.AcceptingBets,
            league,
            0,
            0,
            address(0),
    ...
    function getCampaignInfo(
        uint256 _campaignId
    ) external view virtual override returns (CampaignDetails memory) {
        require(
            _campaignId != 0 && _campaignId <= _totalCampaigns,
            "BettingCampaign: Invalid campaign ID number"
        );
    ...
    function AcceptBets(
        ...
        require(
            _stageConfirmation(_campaignId, Stages.AcceptingBets),
        ...
        campaign_info[_campaignId].campaignBal = campaign_info[_campaignId]
            .campaignBal
            .add(msg.value);
    ...
    function OnwerSetsRaceWinner(
        uint256 _campaignId,
        uint256 racerWhoWon
    ) external virtual override onlyOwner returns (bool) {
        require(
            campaign_info[_campaignId].stage == Stages.Closed,
        ...
        if (_findTheCampaignWinner(_campaignId, racerWhoWon)) {
            ...
            _mint(campaign_info[_campaignId].campaignWinner,racerWhoWon,1,""); //1:1 NFT minted to the  winner of the campaign
        } else {
            campaign_info[_campaignId].stage = Stages.Issue;
        }
        emit RaceWinner(
            _campaignId,
            campaign_info[_campaignId].campaignWinner,
            campaign_info[_campaignId].winnerPayout
        );
        return true;
    }
     ...
     */
    function _findTheCampaignWinner(
        uint256 _campaignId,
        uint256 racerWhoWon
    ) private returns (bool) {
        require(
        ...
        for (uint256 i = 1; i < WinnersArray.length; i++) {
            if (WinnersArray[i].userBetVal > _winningBet) {
                ...
                WinnersArray[i].userBetVal == _winningBet &&
                WinnersArray[i].userBetTimeStamp < _timeStamp
            ) {
                _winningBet = WinnersArray[i].userBetVal;
                _campaignWinner = WinnersArray[i].userAddress;
                _timeStamp = WinnersArray[i].userBetTimeStamp;
        ...
        devFees = (campaign_info[_campaignId].campaignBal)
            .mul(_platformFees)
            .div(100);
        campaign_info[_campaignId].winnerPayout = (
            campaign_info[_campaignId].campaignBal
        ).sub(devFees);
        return true;
    }
    ...
    function WithdrawWinnings(
        ...
        require(
            campaign_info[_campaignId].campaignWinner != address(0),
            "BettingContract: Invalid payout address"
        );
        require(
            _msgSender() == campaign_info[_campaignId].campaignWinner,
            "BettingContract: You didn't win this campaign!"
        );
        ...
        require(
            success,
            "BettingCampaign: Payout to the winner of the campaign failed!"
        );
    ...
    function DevWithdraw() external payable virtual override nonReentrant {
        require(
            devAddress != address(0),
            "Betting Campaign: Owner hasn't set the developer address!"
        );
        require(
    ...
    /**
     *@notice setTokenURI sets the ERC1155 token URI 
     * @param id is the token ID
     */
    function setTokenURI(string memory URI_,uint256 id) public onlyOwner {
        _tokenURI[id] = string(abi.encodePacked(URI_,Strings.toString(id),baseExtension));
        emit TokenURIChanged(_tokenURI[id],id);
    }

        //ERC1155: Change the tokenURI if needed on a per token basis
    /**
     * @notice uri function returns the tokenURI for a certain tokenId
     * @param id of the token
     * @return string which is the tokenURI of the tokenId
     */
